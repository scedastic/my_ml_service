...
from apps.endpoints.models import ABTest, Endpoint, MLAlgorithm, MLAlgorithmStatus, MLRequest
from apps.ml.registry import MLRegistry
from apps.endpoints.serializers import ABTestSerializer, EndpointSerializer, MLAlgorithmSerializer
from apps.endpoints.serializers import MLRequestSerializer, MLAlgorithmStatusSerializer
...
class StopABTestView(views.APIView):
    '''
    Stops the A/B test and computes the accuracy (ratio of correct responses) for each algorithm
    The algorithm with higher accuracy is set as production algorithm and the other 
    is saved with `testing` status
    '''
...
class PredictView(views.APIView):
    def post(self, request, endpoint_name, format=None):
        ...
        if len(algs) != 1 and algorithm_status != "ab_testing":
            return Response(
                {"status": "Error", "message": "ML algorithm select is ambiguous. Please specify algorithm version."},
                status = status.HTTP_400_BAD_REQUEST,
            )
        alg_index = 0
